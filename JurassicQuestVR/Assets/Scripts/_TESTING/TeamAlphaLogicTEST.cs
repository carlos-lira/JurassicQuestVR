...
public class TeamAlphaLogicTEST : LevelManager
{
    ...
    public override void Start()
    {
        ...
        laura = lauraObject.GetComponent<LauraTEST>();
    }
    ...
    void Phase2Logic()
    {
        ...
        else if (!laura.IsTalking() && phase2.audioPlayed && !(phase2.GetDistance(lauraObject.transform.position, phase2.path.GetChild(phase2.path.childCount-1).position) <= 1.5f))
        {
            StopAllCoroutines();
            laura.Run(phase2.path);
        }
    ...
    {
        //Check if Laura is standing and play audio
        if (!laura.IsRunning() && !(phase4.GetDistance(lauraObject.transform.position, phase4.path.GetChild(phase2.path.childCount - 1).position) <= 1.5f))
        {
            StopAllCoroutines();
            laura.Run(phase4.path);
        }
    ...
    public class Phase2 : Phase
    {
        ...
        public bool IsCompleted(int currentKills, GameObject player, GameObject laura)
        {
            if (GetDistance(path.GetChild(path.childCount - 1).position, laura.transform.position) <= 1.5f && GetDistance(laura.transform.position, player.transform.position) <= playerDistanceToLaura)
                completed = true;
            return completed;
        }
    ...
    public class Phase4 : Phase
    {
        ...
        public bool IsCompleted(int currentKills, GameObject player, GameObject laura)
        {
            if (GetDistance(path.GetChild(path.childCount - 1).position, laura.transform.position) <= 1.5 && audioPlayed)
                completed = true;
            return completed;
        }
    }
